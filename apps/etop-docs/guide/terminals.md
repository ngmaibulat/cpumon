...
| **Size** | Panels are dropped whole rather than squeezed. Below 80×19 the dashboard shows one panel at a time and `w` walks them. Below 40×11 it says so and draws nothing else. The tab bar sheds tabs from whichever end is furthest from the one you are on, marking the cut with `‹` or `›`. |
